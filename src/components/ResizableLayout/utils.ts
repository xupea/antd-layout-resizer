...
import type { Coordinate } from "../Resizer";
import { SizeInfo, SizeRelatedInfo } from "./operators";
...
export function isSolid({ size }: Partial<ResizableProps>): boolean {
  return isValidNumber(size);
}

export function isDisabledResponsive(
  childProps: Partial<ResizableProps>
): boolean {
